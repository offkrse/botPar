...
# === Конфиг ===
TOKEN = os.getenv("BOT_TOKEN")  # на Render BOT_TOKEN задашь в Environment

BASE_DATE = datetime(2025, 7, 14)
BASE_NUMBER = 53

# Память для сессий
day_overrides = {}   # {user_id: int} — ручной номер дня
unit_sessions = {}   # {user_id: {"files": [], "day": int}}
...
async def start_unit(message: types.Message):
    """Запускаем режим объединения файлов"""
    user_id = message.from_user.id
...
async def handle_files(message: types.Message):
    ...
    # === Проверяем, есть ли активный режим /unit ===
